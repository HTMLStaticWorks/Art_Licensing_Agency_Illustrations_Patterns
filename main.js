...
function initTestimonialSlider() {
  ...
  if (dotsContainer) {
    ...
    slides.forEach((_, index) => {
      ...
      dot.className = `w-3 h-3 rounded-full transition-all duration-300 ${index === 0 ? 'bg-brand-gold w-6' : 'bg-slate-400/40 dark:bg-slate-700/60'}`;
...
// Interactive Pattern Curation & Catalog Estimator
function initValuationEstimator() {
  const forms = document.querySelectorAll('.valuation-estimator-form');
  forms.forEach(form => {
    const revenueSelect = form.querySelector('.estimator-revenue'); // Desired Pattern Application
    const sectorSelect = form.querySelector('.estimator-sector');   // Target Market Scale
    ...
    form.addEventListener('submit', (e) => {
      ...
      const application = revenueSelect.value;
      const scale = sectorSelect.value;
      
      let accessLevel = "Standard Catalog Access";
      if (scale === 'mass-market') {
        accessLevel = "Enterprise Catalog Access";
      } else if (scale === 'mid-market') {
        accessLevel = "Commercial Catalog Access";
      } else if (scale === 'individual') {
        accessLevel = "Bespoke Collection Key";
      } else {
        accessLevel = "Standard Catalog Access";
      }
      ...
      if (rangeEl) rangeEl.textContent = accessLevel;
      if (sectorEl) {
        const appText = revenueSelect.options[revenueSelect.selectedIndex].text;
        sectorEl.textContent = appText;
      }
